...
// ─────────────────────────────────────────
// Dynamic user attributes (time-series, agent-driven)
// ─────────────────────────────────────────

/**
 * Returns all valid user attributes for a given user (and optionally domain).
 * Results are sorted by recordedAt DESC so callers can take the most recent
 * per (domain, key) using buildAttributeMap() in contextPackBuilder.
 *
 * Only non-expired records are returned (validUntil IS NULL OR >= now).
 */

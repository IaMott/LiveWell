...
import type { ToolExecutionContext, ToolExecutionHandler } from './toolExecutor'
...
// Known scalar profile fields that should also be written as UserAttribute entries
// for time-series history whenever user.updateProfile is called with them.
const PROFILE_SCALAR_TO_ATTRIBUTE: Record<string, { domain: string; key: string; unit?: string }> =
  {
...
/**
 * user.updateProfile — legacy flat-merge handler for profile UI blobs.
 * Also creates UserAttribute history entries for known scalar fields
 * (weight, height) so history is preserved even when agents use this older tool.
 */
const userUpdateProfile: Handler = async (args, ctx) => {
  ...
  // Side-effect: create UserAttribute entries for any known scalar fields
  // so we accumulate history even via the old tool.
  const attrWrites: Promise<unknown>[] = []
  for (const [profileKey, value] of Object.entries(fields)) {
    const mapping = PROFILE_SCALAR_TO_ATTRIBUTE[profileKey]
    if (mapping && (typeof value === 'number' || typeof value === 'string')) {
      attrWrites.push(
  ...
  if (attrWrites.length > 0) await Promise.all(attrWrites)

  return { saved: true }
}

/**
 * user.setAttribute — the primary agent tool for dynamic time-series data.
 * Always creates a NEW record (never overwrites). Full history is preserved.
 *
 * Agents use this to store:
 * - Measurements: weight, height, blood_pressure, heart_rate_resting, bmi
 * - Clinical: diagnosis, medication, allergy, injury, chronic_condition
 * - Lifestyle: diet_preference, sleep_hours, stress_level, energy_level
 * - Goals: any goal with horizon, priority, status
 * - Any domain-specific attribute discovered during conversation
 */
const userSetAttribute: Handler = async (args, ctx) => {
  ...
  const entry = await prisma.userAttribute.create({
    data: {
      userId: ctx.actor.userId,
      domain: a.domain,
      key: a.key,
  ...
  // For known scalar fields, also keep UserProfile in sync (UI display)
  const profileKey = a.domain === 'personal' ? a.key : null
  if (profileKey && (typeof a.value === 'number' || typeof a.value === 'string')) {
    const profileField =
      profileKey === 'weight'
        ? { weight: a.value }
        : profileKey === 'height'
          ...
          : null
    if (profileField) {
      await updateUserProfile(ctx.actor.userId, profileField as Record<string, unknown>).catch(
        () => {},
      )
    }
  }
...
// share.createLink is destructive + owner-mode

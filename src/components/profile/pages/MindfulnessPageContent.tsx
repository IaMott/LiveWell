...
function MindfulnessForm({
  ...
  return (
    <ProfileForm onSubmit={handleSubmit} saving={saving} error={error} success={success}>
      <div>
        <label htmlFor="stressLevel" className={labelClass}>
      ...
      <div>
        <label htmlFor="meditationExp" className={labelClass}>
          Esperienza con la meditazione
        </label>
        <select
          id="meditationExp"
          value={form.meditationExp}
          onChange={(e) => setForm({ ...form, meditationExp: e.target.value })}
          className={selectClass}
        >
      ...
      <div>
        <label htmlFor="techniques" className={labelClass}>
          Tecniche di rilassamento conosciute
        </label>
        <textarea
          id="techniques"
          rows={2}
          ...
          placeholder="Es. respirazione diaframmatica, yoga, journaling..."
        />
      </div>
    </ProfileForm>
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCompletenessHint
        sectionData={getSectionCompleteness('mindfulness')}

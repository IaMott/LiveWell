...
const KEY_LABELS: Record<string, string> = {
  ...
  daily_kcal: 'Calorie giornaliere',
  protein_g: 'Proteine',
  carbs_g: 'Carboidrati',
  fat_g: 'Grassi',
  dietary_restrictions: 'Restrizioni dietetiche',
  budget_food: 'Budget spesa',
  cooking_time: 'Tempo cottura',
}
...
export function NutritionSection({ data }: Props) {
  ...
  const entries = Object.entries(history['nutrition'] ?? {})

  const assessments = dedupAssessments(
    (clinicalEvents ?? []).filter(
      (e) => e.eventType === 'agent_assessment' && e.domain === 'nutrition',
    ),
  )

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <p style={titleStyle}>Nutrizione</p>
